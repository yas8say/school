      ...
      <header class="md:hidden bg-white shadow-sm border-b border-gray-200 p-4 flex items-center justify-between">
        <div class="flex items-center space-x-3">
          ...
          <div class="min-w-0 flex-1">
            <h2 class="text-sm font-semibold text-gray-800 truncate">{{ parent.name || 'Parent' }}</h2>
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto space-y-6 md:space-y-8">
          ...
          <div class="hidden md:flex items-start space-x-4 border-b pb-4">
            ...
            <div class="min-w-0 flex-1">
              <h2 class="text-xl font-semibold text-gray-800 break-words">{{ parent.name || 'Loading...' }}</h2>
              <p class="text-sm text-gray-500 mt-1">Parent</p>
              <p v-if="parent.address" class="text-xs text-gray-600 mt-1 truncate">{{ parent.address }}</p>
            </div>
          </div>
          ...
          <div class="space-y-2">
            ...
            <select 
              ...
              :disabled="userDetailsResource.loading || !studentList.length"
            >
            ...
            <p v-if="!studentList.length" class="text-sm text-yellow-600">
          ...
          <div class="space-y-3">
            ...
            <nav class="space-y-2">
              ...
              <button
                ...
                :disabled="!selectedStudent || userDetailsResource.loading"
              >
                <ClipboardDocumentListIcon class="w-5 h-5 flex-shrink-0" />
                <span class="truncate">Attendance Record</span>
              </button>
              ...
              >
                ...
                <span class="truncate">Browse Notices</span>
              </button>
              ...
              >
                ...
                <span class="truncate">Appeal Leave</span>
              </button>
            </nav>
          </div>
        </div>
      ...
      <main class="flex-1 min-h-screen md:min-h-0 p-4 md:p-6 lg:p-8 bg-gray-50" :class="{ 'opacity-50 pointer-events-none': userDetailsResource.loading }">
        ...
        <div v-if="activeView === 'dashboard'">
          ...
          <div class="bg-white rounded-lg shadow-sm p-4 md:p-6 mb-4 md:mb-6">
            <h1 class="text-xl md:text-2xl font-bold text-gray-800 mb-2">Welcome, {{ parent.name || 'Parent' }}!</h1>
            <p class="text-gray-600 text-sm md:text-base">Monitor your child's activities and school records from here.</p>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-4 md:mb-6">
            <div class="bg-white rounded-lg shadow-sm p-4 md:p-6 border-l-4 border-blue-500">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-xs md:text-sm font-medium text-gray-600">Selected Child</p>
                  <p class="text-lg md:text-2xl font-bold text-gray-800 truncate">{{ selectedStudent?.student_name || 'None' }}</p>
                  <p v-if="selectedStudent?.student_group" class="text-xs text-gray-500 mt-1">Class: {{ selectedStudent.student_group }}</p>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 md:p-6 border-l-4 border-green-500">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-xs md:text-sm font-medium text-gray-600">Available Children</p>
                  <p class="text-lg md:text-2xl font-bold text-gray-800">{{ studentList.length }}</p>
                </div>
                <UserGroupIcon class="w-6 h-6 md:w-8 md:h-8 text-green-500 flex-shrink-0" />
              </div>
            </div>
          ...
          <div class="bg-white rounded-lg shadow-sm p-4 md:p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  <h3 class="font-medium text-gray-800 text-sm md:text-base">View Attendance Record</h3>
                  <p class="text-xs md:text-sm text-gray-600 truncate">Check your child's attendance history</p>
                </div>
              </button>
              ...
              >
                <InboxIcon class="w-5 h-5 md:w-6 md:h-6 text-yellow-600 mr-3 flex-shrink-0" />
                <div class="min-w-0 flex-1">
                  <h3 class="font-medium text-gray-800 text-sm md:text-base">Appeal for Leave</h3>
                  <p class="text-xs md:text-sm text-gray-600 truncate">Submit leave appeal for your child</p>
                </div>
              </button>
            </div>
            <p v-if="!selectedStudent" class="text-sm text-yellow-600 mt-3">
              Please select a child to access these features
            </p>
          </div>
        </div>
        ...
        <div v-else class="h-full">
          <component 
            ...
            :key="selectedStudent?.student + activeView"
          />
        </div>
      </main>
    </div>
...
const componentMap = {
  ...
  'browse-notices': BrowseNotices,
  'appeal-leave': AppealLeave
}
...
const loadUserDetailsFromLocalStorage = () => {
  try {
    ...
    if (parent.value.name || studentList.value.length) {
      hasInitialData.value = true
      globalError.value = null
    }
  } catch (error) {
