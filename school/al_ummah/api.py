...
@frappe.whitelist()
def onboard_instructor(source_name):
    # Real-time progress (Step 1/2)
    frappe.publish_realtime(
        "convert_to_instructor_progress", {"progress": [1, 2]}, user=frappe.session.user
    )

    # Map Employee Applicant to Employee
    instructor = get_mapped_doc(
        ...
        source_name,
        {
            "Instructor Applicant": {
                "doctype": "Instructor",
                "field_map": {
                    "name": "instructor_applicant",  # Special mapping
                },
            }
        },
        ignore_permissions=True,
    ...
    # Real-time progress (Step 2/2)
    frappe.publish_realtime(
        "onboard_instructor_progress", {"progress": [2, 2]}, user=frappe.session.user
    )
...
@frappe.whitelist()
def get_instructor_app_data(student=None, date=None):
...
def submit_leave_application(student, from_date, to_date, student_group, reason, image=None):
    print("___________________________________________", student)
...
def submit_notice(notice_heading, notice_message, student_group):
    ...
    send_notice_notification_to_app(student_list, notice_message, notice_heading)
    try:
...
def get_notice_list(student_group):
    print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", student_group)
