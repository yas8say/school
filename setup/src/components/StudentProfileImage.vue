    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="w-16 h-16 rounded-full border-2 border-gray-100 shadow-sm overflow-hidden bg-gray-50 cursor-pointer" 
        ...
        <img v-if="profileImageUrl" 
             ...
             loading="lazy" />
        <div v-else 
             class="w-full h-full flex items-center justify-center text-white font-semibold text-xl"
    ...
    <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
      <div class="flex items-center space-x-4">
        <div class="relative cursor-pointer" @click="openImagePreview(profileImageUrl)">
          ...
          <div v-if="profileImageUrl" class="absolute bottom-0 right-0 bg-blue-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs">
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              <div class="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center cursor-pointer" 
                ...
                <div v-else 
                     class="w-full h-full flex items-center justify-center text-blue-600 font-semibold text-sm rounded-lg bg-blue-50">
                  {{ avatarText }}
                </div>
              </div>
              <div>
        ...
        <div class="flex-1 min-h-0 bg-gray-100 rounded-lg p-4 overflow-hidden">
          ...
          <div v-else class="h-full w-full flex items-center justify-center">
            <div class="text-center">
              ...
              <p class="text-gray-600">Loading image editor...</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex-shrink-0 mt-6">
          <div class="flex justify-center items-center space-x-4 mb-6">
            <button type="button" @click="rotate(-90)" 
                    :disabled="!cropperLoaded"
                    class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 disabled:bg-gray-100 disabled:text-gray-400 flex items-center space-x-2 transition-colors" 
                    title="Rotate Left">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200">
            ...
            <button 
              ...
              @click="applyCrop"
              :disabled="!cropperLoaded"
              class="px-8 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-green-300 font-medium transition-colors"
            >
              Apply Crop
            </button>
          </div>
...
// Lazy load CropperJS
const VueCropper = shallowRef(null)
const cropperLoaded = ref(false)

const loadCropper = async () => {
  if (VueCropper.value) return
  
  try {
    const cropperModule = await import('vue-cropperjs')
    VueCropper.value = cropperModule.default
    ...
    emit('error', 'Failed to load image editor')
  }
}
...
const profileImageUrl = computed(() => {
  ...
  // Priority 2: Preview image (during upload process)
  if (imagePreview.value) {
    return imagePreview.value
  }
  // Priority 3: Existing image from database
  if (props.currentImage && props.currentImage !== 'null' && props.currentImage !== '') {
    return getProfileImageUrl(props.currentImage)
  }
  // Priority 4: No image - show avatar
  return null
})
...
const avatarText = computed(() => {
  ...
  const names = props.studentName.trim().split(' ')
...
const avatarColor = computed(() => {
  if (!props.studentName) return '#6b7280'
  ...
  let hash = 0
  for (let i = 0; i < props.studentName.length; i++) {
    hash = props.studentName.charCodeAt(i) + ((hash << 5) - hash)
  }
...
const openImagePreview = (imageUrl) => {
  // Don't open preview for avatar (when no image exists)
  if (!imageUrl) return
...
/* ---------- CROPPER ---------- */
const rotate = (deg) => {
  if (cropper.value && cropperLoaded.value) {
...
const applyCrop = () => {
  if (!cropper.value || !cropperLoaded.value) return
...
const handleImageError = (e) => {
  // Hide the broken image and let avatar show
  e.target.style.display = 'none'
  
  // Find the parent container and show avatar instead
  const parent = e.target.parentElement
  if (parent) {
    const avatarDiv = document.createElement('div')
    avatarDiv.className = 'w-full h-full flex items-center justify-center text-white font-semibold text-2xl'
    avatarDiv.style.backgroundColor = avatarColor.value
    avatarDiv.textContent = avatarText.value
    
    // Replace the broken image with avatar
    parent.innerHTML = ''
